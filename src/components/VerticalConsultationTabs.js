...
export default function VerticalTabs({
  ...
  const handleBookAppointment = () => {
    const payload = {
      ...
      consultationId: selectedTab?.consultation?.id,
      ...
      >
        <div>
          <p className="text-2xl font-semibold text-secondary flex items-center py-5 gap-4">
            ...
            Schedule Appointment (#{selectedTab?.consultation?.id})
          </p>
          <AppoinmentForm
      ...
      >
        {items?.map((consultation, index) => (
          <Tab
            key={index}
            name="consultation"
            label={`#${consultation.id}-${consultation.type}`}
            {...a11yProps(index)}
          />
        ))}
      </Tabs>
          ...
          <div className="flex flex-wrap p-5 gap-2 overflow-y-auto h-44">
            ...
            <Button
              ...
              onClick={e => {
                ...
                setAppointmentForm({ consultationId: consultation.id })
              }}
            >
              <FaPlusCircle />
